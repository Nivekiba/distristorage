...
for i in $(seq 1 3 10); do
        ...
        ssh -o StrictHostKeyChecking=no \
                root@${nodes[0]} \
                "sudo /mnt/qcow2-snapshots/expes/footprint.exp /mnt/qcow2-snapshots/qemu-4.2-vanilla/build /mnt/qcow2-snapshots/expes/disk/ub_tmp0.qcow2 6M" > /users/nivekiba/dd$i"0" &
        pidd=$!
        echo $pidd >> /tmp/order
        pid_arr+=($pidd)
        ...
        for j in $(seq 1 $i); do
                ...
                ssh -o StrictHostKeyChecking=no \
                        root@${nodes[0]} \
                        "sudo /mnt/qcow2-snapshots/expes/footprint.exp /mnt/qcow2-snapshots/qemu-4.2-vanilla/build /mnt/qcow2-snapshots/expes/disk/ub_tmp$j.qcow2 6M" > /users/nivekiba/dd$i$j &
                pidd=$!
                echo $pidd >> /tmp/order
                pid_arr+=($pidd)
        done
